...
namespace WebStore.WebAPI.Clients.Identity
{
    class UsersClient:BaseClient,IUsersClient
    {
        ...
        public async Task<IdentityResult> DeleteAsync(User user, CancellationToken cancel)
        {
            ...
            var delete_success = await response
                .EnsureSuccessStatusCode()
                .Content
                .ReadFromJsonAsync<bool>(cancellationToken: cancel)
                .ConfigureAwait(false);
        ...
        {
            return await GetAsync<List<User>>($"{Address}/UserInRole/{role}", cancel).ConfigureAwait(false);
        }

        #endregion

        #region Imlementation of IUserPasswordStore<User>
        ...
        #region Imolementation of IUserPhoneNumberStore<User>
        ...
        #region Implemenatation of IUserTwoFactorStore<User>
        ...
        {
            await PostAsync(
                    $"{Address}/ReplaceClaims",
                    new ReplaceClaimDTO {User = user, Claims = claims},
                    cancel)
                .ConfigureAwait(false);
        }
        ...
        {
            var response = await PostAsync($"{Address}/GetUsersClaim", claim, cancel).ConfigureAwait(false);
            return await response
                .EnsureSuccessStatusCode()
                .Content
